"""Integration tests for the Liability → Foundry bridge.
...
from argos.services.foundry.liability_bridge import (  # noqa: E402
...
def test_bridge_raises_when_flag_set_but_env_missing(monkeypatch):
    ...
    with pytest.raises(FoundryBridgeNotConfigured):
        propagate_liability_decision_to_foundry(claim_id="CLM-001", accept=True)


def test_source_assessment_id_is_optional(monkeypatch):
    monkeypatch.delenv("ARGOS_FOUNDRY_BRIDGE_ENABLED", raising=False)
    assert (
        propagate_liability_decision_to_foundry(claim_id="CLM-001", accept=False)
        is None
    )
...
def test_bridge_round_trip_records_operation_id(monkeypatch):
    ...
    apply-liability-decision and a Claim row in the new ontology."""
    monkeypatch.setenv("ARGOS_FOUNDRY_BRIDGE_ENABLED", "1")
    reset_client_cache()

    op_id = propagate_liability_decision_to_foundry(
        claim_id="CLM-001",
        accept=True,

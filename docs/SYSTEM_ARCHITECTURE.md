...
| **Foundry bridge layer** | infra | **live-verified 2026-06-04 — 6/6 bridges round-trip** against the live Argos ontology. All 6 workflow bridges (Coverage, Reserve, Liability, Recovery, Closure with closure+reopen, AgentAction emission) verified. 25/25 tests green (19 unit + 6 live integration against seeded `CLM-001`). Bridges via Python OSDK `argos_live_sdk` v0.2.0. INVALID-validation check mandatory post-call on every bridge. AgentAction bridge wired into `audit_log.py::append_agent_action` — declarative-rule mirror only, citations stay local until function-backed upgrade (deferred with pre-baked TypeScript). | [`services/foundry/`](../src/argos/services/foundry/), [`docs/architecture/foundry-bridge-pattern.md`](./architecture/foundry-bridge-pattern.md) |

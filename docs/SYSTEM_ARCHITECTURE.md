...
| **Foundry bridge layer** | infra | **Wiring shipped; live write currently BLOCKED (2026-06-07).** Bridge code + the API decision→handler→bridge path are correct and verified by mocked default-suite tests; asymmetric-commit degrades gracefully. BUT the installed OSDK `argos_live_sdk` **v0.2.0 is bound to the legacy ontology `d7926c75-…`** and every live action returns `ActionTypeNotFound` — so the "6/6 live-verified 2026-06-04" claim (which used **v0.1.0** bound to `88f01e1f-…`) is **stale; currently 0/6 live**. Fix is external: regenerate/pin `argos_live_sdk` to `88f01e1f-…` + valid token, then `pytest -m foundry_integration`. Undetected because those tests are `addopts`-excluded from the default suite (see DECISIONS 2026-06-07). | [`services/foundry/`](../src/argos/services/foundry/), [`docs/architecture/foundry-bridge-pattern.md`](./architecture/foundry-bridge-pattern.md) |
...
| **FastAPI service** (cockpit surface) | infra | shipped 2026-06-07 — `GET /api/claims`, `/claims/{id}` (with live dossier), `/metrics`, `/demo/examples`; `POST /run/{workflow}`, `/decisions` (routes to orchestrator handlers), `/demo/seed-claim`. Bearer auth via `ARGOS_DEMO_TOKEN` (skipped if unset). | [`api/app.py`](../src/argos/api/app.py), [`api/mappers.py`](../src/argos/api/mappers.py), [`api/schemas.py`](../src/argos/api/schemas.py) |
...
| **Next.js cockpit** | infra | shipped 2026-06-07 — runs LIVE on the FastAPI surface (caseload, metrics, decision-commit, per-stage dossier, document viewer). Local: backend `:8071` + cockpit `:3007`. Hosted (Vercel + deployed backend) pending. Eval-safe demo data via [`ontology/cockpit_caseload.py`](../src/argos/ontology/cockpit_caseload.py). | [`web/`](../web/) (`lib/api.ts`, `components/App.tsx`) |
...
5. ~~**Reserve/Liability/Recovery/Closure Foundry bridges + AgentAction emission**~~ — **LIVE-VERIFIED 2026-06-04 (6/6).** All 5 workflow bridges plus the AgentAction emission bridge round-trip against the live Argos ontology; 25/25 tests green. Package migrated `argos_osdk_sdk` → `argos_live_sdk` v0.2.0 (correct ontology binding). INVALID-validation check added to the bridge contract. Function-backed `emit-agent-action` upgrade for atomic AgentAction + EvidenceCitation materialization deferred with pre-baked TypeScript source in [`docs/architecture/foundry-bridge-pattern.md`](./architecture/foundry-bridge-pattern.md) — citations stay local until that lands.

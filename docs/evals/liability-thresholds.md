...
|---|---|---|---|---|
| 1 | `vicarious_cap_value` semantics (per-person vs per-occurrence) ambiguous — calc emits per-occurrence `$300K`, schema field name implies per-person. | RESOLVED 2026-06-02 (307ec2d) | — | Documented the per-occurrence semantics in the `ExposureCeiling.vicarious_cap_value` field docstring; future per-person needs surface as a separate field rather than overloading this one. Reserve eval confirmed it consumes `policy_limits.per_person` directly, not this field. |
...
| 5 | Subrogation rides `liability` posture (Document Reader eval is locked). | RESOLVED 2026-06-02 | — | Code-path shipped: `PostureChanged` literal extended to include `subrogation`; Document Reader prompt + exemplar added; dispatcher routes `subrogation → [recovery]`; dispatcher unit test asserts the routing. Layer-1 anchor-pair eval (Pairs 5–7: consent-to-settle, AF eligibility, made-whole waiver) is locked in [`document-reader-anchor-pairs-v3-subrogation-thresholds.md`](./document-reader-anchor-pairs-v3-subrogation-thresholds.md), pending live-API execution. |

...
|---|---|---|---|---|
| 1 | `made_whole_shortfall` math is force-zeroed (`shortfall = Decimal("0")`) after computation — the field surfaces in the ledger but doesn't reduce `basis`. | RESOLVED 2026-06-02 (307ec2d) at the docstring level | Real case requires per-lane subtraction | Field now docstring-labeled INFORMATIONAL ONLY in v1; `basis` docstring updated to match the actual math (no shortfall subtraction). Further work — gating subtraction by subrogation lane — deferred until a real case needs it. |
